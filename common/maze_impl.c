...
static float quantize_to_right_angle(float yaw) {
  // Snap to nearest multiple of 90° (pi/2)
  const float q = (float)M_PI_2;
  int k = (int)lroundf(yaw / q);
  return normalize_angle((float)k * q);
}
...
static void start_turn(float current_yaw, turn_cmd_t cmd) {
  float snapped = quantize_to_right_angle(current_yaw);
  float delta = 0.0f;
  switch (cmd) {
    case TURN_RIGHT: delta = -(float)M_PI_2; break;
    case TURN_LEFT:  delta = +(float)M_PI_2; break;
    case TURN_BACK:  delta = +(float)M_PI;   break;
    default:         delta = 0.0f;          break;
  }
  s_turn_target_yaw = normalize_angle(snapped + delta);
